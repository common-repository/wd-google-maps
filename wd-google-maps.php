...
function gmwd_bp_install_notice()
{
    ...
    if ($meta_value === '' || $meta_value === false) {
        ...
        <script>wd_bp_url = '<?php echo add_query_arg(array('action' => 'wd_bp_dismiss', 'nonce_gmwd' => wp_create_nonce('nonce_gmwd')), admin_url('admin-ajax.php')); ?>'</script>
        <?php
        echo ob_get_clean();
    }
}
...
function gmwd_bp_install_notice_status()
{
  if (function_exists('current_user_can')) {
    $nonce = isset($_GET['nonce_gmwd']) ? esc_html($_GET['nonce_gmwd']) : '';
    if ( $nonce != '' && current_user_can('manage_options') && wp_verify_nonce( $nonce, 'nonce_gmwd' ) ) {
